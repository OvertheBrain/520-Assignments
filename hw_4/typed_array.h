...
TypedArray<ElementType> TypedArray<ElementType>::concat(const TypedArray& other) {
    ...
    for(int j = 0; j < other.size(); j++){
        res.set(j + size(), other.safe_get(j));
    }
...
// The '+' operator for the TypedArray class which means concatenation. 
template <typename ElementType>
TypedArray<ElementType> operator+(TypedArray<ElementType> a, TypedArray<ElementType> &b){
    return a.concat(b);
}
